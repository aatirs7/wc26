          ...
          <li key={row.ownerId}>
            ...
            {open ? (
              <div className="mt-1.5 rounded-xl border border-edge bg-white/[0.02] p-3 text-sm">
                <div className="mb-1.5 text-center text-[0.65rem] font-bold uppercase tracking-[0.2em] text-muted-2">
                  Where the points come from
                </div>
                {row.rounds.length === 0 && row.bonus === 0 ? (
                  <p className="text-center text-xs text-muted">No points scored yet.</p>
                ) : (
                ...
                {row.bracketId ? (
                  <div className="mt-2 text-center">
                    <Link
                      ...
                      className="inline-block text-xs font-bold text-accent"
                    >
                      View full bracket →
                    </Link>
                  </div>
